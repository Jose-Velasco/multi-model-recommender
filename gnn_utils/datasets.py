...
class PositiveIterationGraph(InMemoryDataset):
    ...
    def process(self):
        ...
        with file_path.open("r") as f:
            ...
            for line in f:
                ...
                # # users_ids.add(line_list[0])
                # # item_ids.update(line_list[1:])
                # current_user_id = line_list[0]
                # if current_user_id in users_ids:
                #     raise KeyError(f"Error: user_id '{current_user_id}' already has a user_node_id of {users_ids[current_user_id]}. Possible duplicate user.")
                # users_ids[current_user_id] = node_id
                # node_id += 1
                # for item_id in line_list[1:]:
                #     if item_id in item_ids:
                #         raise KeyError(f"Error: item_id '{item_ids}' already has a item_node_id of {item_ids[item_id]}. Possible duplicate item.")
                #     item_ids[item_id] = node_id
                #     node_id += 1
                #     # appends the current users node id as source node
                #     # user_item_edges[0].append(users_ids[current_user_id])
                #     # appends the current item node id as destination node
                #     # user_item_edges[1].append(item_ids[item_id])
        ...
        edges = torch.tensor(user_item_edges, dtype=torch.int32)
        graph = Data(
            x=nodes,
            edge_index=edges
        )
    ...
    def _process_line(self, line: str, users_ids: dict[int, int], item_ids: dict[int, int], user_item_edges: list[list]) -> None:
        ...
        # if current_user_id in users_ids:
        #     raise KeyError(f"Error: user_id '{current_user_id}' already has a user_node_id of {users_ids[current_user_id]}. Possible duplicate user.")
        # users_ids[current_user_id] = self._processing_node_idx_counter
        # self._processing_node_idx_counter += 1
        # add assigns user_id to a node_id
        self._insert_into_entity_id_node_id_map(entity_id=current_user_id, entity_to_node_id_map=users_ids)
        for item_id in line_list[1:]:
            # if item_id in item_ids:
            #     raise KeyError(f"Error: item_id '{item_ids}' already has a item_node_id of {item_ids[item_id]}. Possible duplicate item.")
            # item_ids[item_id] = self._processing_node_idx_counter
            # self._processing_node_idx_counter += 1
